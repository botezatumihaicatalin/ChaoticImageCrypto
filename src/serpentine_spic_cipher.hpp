...
class serpentine_spic_cipher: public spic_cipher<spectrum> {

private:
  serpentine_spic_key* key_;

public:
  serpentine_spic_cipher() : key_(nullptr) {}
...
template <size_t spectrum>
void serpentine_spic_cipher<spectrum>::init_key(const serpentine_spic_key& key) {
  key_ = new serpentine_spic_key(key);
}

template <size_t spectrum>
inline uint8_t* serpentine_spic_cipher<spectrum>::encrypt(uint8_t* pixels, uint32_t size) const {
  if (key_ == nullptr) {
...
template <size_t spectrum>
inline uint8_t* serpentine_spic_cipher<spectrum>::decrypt(uint8_t* pixels, uint32_t size) const {
  if (key_ == nullptr) {
